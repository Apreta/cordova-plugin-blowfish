/*
	...
	Apreta: byte ordering changed to match existing clients (little endian format)
...
public class BlowfishECB
{
	...
	private void init(byte[] key, int nOfs, int nLen)
	{
		m_blockBuf = new byte[BLOCKSIZE];
	...
	protected int encryptPrv(
		...
		int nLen)
	{
		int extra = nLen % BLOCKSIZE; 

		int encLen = nLen - nLen % BLOCKSIZE;

		long lTemp;

		int nC = nInPos + encLen;
